...
//						'choices' => apply_filters('get_leadlovers_sequence_list', array()) //os produtos serão buscados na função no arquivo do plugin
					...
					array(
						...
						'type'              => 'text',
						'name'              => 'gf_leadlovers_tag_prefix',
						//'tooltip'           => esc_html__( 'O ', 'gravityforms-ll' ),
						'class'             => 'medium',
						//'feedback_callback' => array( $this, 'is_valid_setting' ),
					),
					array(
						'label'   => esc_html__( 'Inserir a Semana do Ano na Tag', 'gravityforms-ll' ),
						'type'    => 'checkbox',
						'name'    => 'week_tag_enable',
						'tooltip' => esc_html__( 'Caso habilitado, o sistema verificará a Semana do Ano atual [1 - 53] e adicionará após à TAG acima.', 'gravityforms-ll' ),
						'choices' => array(
							array(
								'label' => esc_html__( 'Habilitar a semana do Ano', 'gravityforms-ll' ),
								'name'  => 'week_tag_enabled',
	...
	public function after_submission( $entry, $form ) 
	{
		...
		//Pega o prefico da Tag e junta com o numero da atual semana do ano
		$tag = $settings['gf_leadlovers_tag_prefix'];
		//Se estiver habilitado o uso da tag da semana do ano
		if($settings['week_tag_enabled'])
		{
			$tag .= date_i18n( 'W' ); //Prefixo + Semana do ano
		}
		//$product = $settings['gf_leadlovers_product_choice'];
		...
		{
			// //DEBUG
			// $console = "<script>console.log(' Mensagem do além: Boooo!!! ');</script>";
			// echo $console;
	
			// caso exista de fato uma tag 
			// será preciso capturar seu id pela API da LeadLovers
			if($tag)
			{
				// // DEBUG
				// $console = "<script>console.log(' Mensagem do além: " . $tag . " - ". $tag_id . "');</script>";
				// echo $console;
				...
				{
					// // DEBUG
					// $console = "<script>console.log(' TAG: " . $tag . "');</script>";
					// echo $console;
					...
					{
						// Verifica se o item corresponde ao nome da tag
						if($item['Title'] == $tag)
						{
							// Conseguimnos o ID da tag
							$tag_id = $item['Id'];
							break;
						}
					}
					if(!$tag_id) // Se não há um tag_id é pq não encontrou
					{
						$messages .= 'Tag não encontrada na lista de tags...<br>';
						$error = true;
					}
				}
				else
			...
			// // DEBUG
			// $console = "<script>console.log(' Mensagem do além 2: " . $tag . " - ". $tag_id . "');</script>";
			// echo $console;
		...
		{
			//DEBUG
			// $console = "<script>console.log(' Cadastrando no produto... ');</script>";
			// echo $console;
		...
		public function get_leadlovers_machine_list()
		{
			...
			if ( 200 == $status ) 
			{
				...
				if(!$machine_id)
				{
					$machine_list[] = array('label' => 'Selecione uma Máquina', 'value' => '1');
				}
			...
			{
				return array('label' => 'erro', 'value'=>'1');
			}
		}
		...
		public function get_leadlovers_sequence_list_ajax_callback()
		{
			if(isset($_REQUEST))
			{
				$response = $this->get_leadlovers_sequence_list($_REQUEST['machine_id']);

// #### SAVE THE NEW CHOICES ####
				$form = $this->get_current_form();
				$settings = $this->get_form_settings( $form );
				$settings['gf_leadlovers_sequence']['choices'] = $machine_list;
				$this->_save_form_settings($form, $settings); // <<<--- THIS BROKES THA AJAX
				...
				echo $sequence_list_html;

			}
			else
			{
				// DEBUG
				$console = "<script>console.log(' Deu ruim: " . get_type($_REQUEST) . "');</script>";
				echo $console;
		...
		public function get_leadlovers_level_list_ajax_callback()
		{
			...
			if(isset($_REQUEST))
			{
				$response = $this->get_leadlovers_level_list($_REQUEST['sequence_id'],$_REQUEST['machine_id']);
